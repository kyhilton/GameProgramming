...
public class StudyStringThree : MonoBehaviour
{
    ...
    void addANote(int rank)
    {
        ...
        Vector3 newPosition = new Vector3(cen.transform.position.x  + ((rank-2/2) * scaleFactor), cen.transform.position.y, cen.transform.position.z);
        GameObject n = (GameObject)(Instantiate(note, newPosition, Quaternion.identity));
        
         n.tag = "" + rank;
         n.name = "" + rank;
         string nameOfNote = "";
         string stringNumber = "_3";
         string fileName = "";
