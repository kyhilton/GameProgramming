...
public class Level3Tests: InputTestFixture
{
    ...
    {
        SceneManager.LoadScene("playLevel3");
        base.Setup();
    ...
    {
        base.TearDown();
        SceneManager.LoadScene("playLevel3"); // Unloading the last loaded scene Assets/Resources/test_scene.unity(build index: 0), is not supported. Please use SceneManager.LoadScene()/EditorSceneManager.OpenScene() to switch to another scene.
    }
    ...
    public IEnumerator AnswerSelected()
    {
        ...
        GameObject AnswerSelected = GameObject.Find("gameManager").GetComponent<ManageStaffAndFrets3>().retrieveSelectedAnswer();
    ...
    public IEnumerator ScoreIncreases()
    {
        ...
        int score = GameObject.Find("gameManager").GetComponent<ManageStaffAndFrets3>().retrieveScore();
