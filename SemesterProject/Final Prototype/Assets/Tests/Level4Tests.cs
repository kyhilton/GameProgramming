...
public class Level4Tests: InputTestFixture
{
    ...
    {
        SceneManager.LoadScene("playLevel4");
        base.Setup();
    ...
    {
        base.TearDown();
        SceneManager.LoadScene("playLevel4"); // Unloading the last loaded scene Assets/Resources/test_scene.unity(build index: 0), is not supported. Please use SceneManager.LoadScene()/EditorSceneManager.OpenScene() to switch to another scene.
    }
    ...
    public IEnumerator AnswerSelected()
    {
        ...
        GameObject AnswerSelected = GameObject.Find("gameManager").GetComponent<ManageStaffAndFrets4>().retrieveSelectedAnswer();
    ...
    public IEnumerator ScoreIncreases()
    {
        ...
        int score = GameObject.Find("gameManager").GetComponent<ManageStaffAndFrets4>().retrieveScore();
